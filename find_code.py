...
def find_code_all():
    # 把这个函数的输出都保存到txt文件中去
    with open("train_no.txt", "w", encoding="utf-8") as f:
        cnt = 0
        for i in range(1, 9999):
            ...
            for prefix in "GDCZTKSYP":
                code = prefix + str(i)
                if code in no_list:
                    f.write(code + '\t' + train_list[no_list[code]][0]["start_station_name"] + ' - ' +
                          train_list[no_list[code]][0]["end_station_name"] + "\n")
                    flag = True
                    break
            if str(i) in train_list:
                ...
                flag = True
            if not flag:
                f.write(str(i) + "not found" + "\n")
                cnt += 1
        f.write("not found sum:" + str(cnt))
...
def rank_station(num=200, key="动分动集普速", prefix="GDCZTKSYP"):
    rank = {}
    detail = {}
    for i in train_list:
        # print(train_list[i])
    ...
    for rank, (station, train_num) in enumerate(rank):
        print(str(rank + 1).rjust(4, ' '), station.ljust(5, ' '), "\t共计 ", train_num, end='  ')
...
while s != "exit":
    ...
    if "rank_station" in s.lower():
        ...
        if '*' in key_and_prefix:
            ...
            rank_station(num=100, key=key, prefix=prefix)
        else:
            rank_station(num=100, key=s[13:])
        continue
