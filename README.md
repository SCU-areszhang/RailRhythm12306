...
### **这篇文档比较详细，快速上手建议阅读[简明指南](global_data/Simple%20user%20file.md)。**
...
**~~感觉现在很难完整爬下来了 是只有我电脑这样吗 12306怎么这么坏 我就拿它练习一下爬虫 不要封我啊啊啊啊~~**
...
需要说明的是，**有部分车次的时刻表无法通过时刻表查询得到，大约占全部的2-3%**。这些车次大部分实际上确实会开行，但是直接从12306主页搜索框里却无法得到存储其时刻表的url，范围不定，原因未知，**暂无修复思路**。
...
Train sum:	 15154 	( 10484 )
    ...
    Y prefix	 14 	( 5 )
    Pure number	 329 	( 262 )
# 导入后共有15154条车次信息，去除车次切换和当日不开行后是10484条
```
这是正常导入。
...
在测试程序的时候你也可以读取我已经导入的数据。例如，仓库中`train_data/no_list20250223.json`和`train_data/train_list20250223.json`两个文件包括了2025年2月23日的全部车次数据（其他几个日期的数据可能并不完整）。

### 2.2 时间设置

本工具并不支持直接处理归属于不同日期的数据，但是可以采用下面给出的间接办法。

按照`2025-01-01`的格式输入一个日期，或者先输入命令`Time`或`Date`再输入日期，可以更改日期设置。

数据管理的底层逻辑是这样的：每次使用命令`Load`，就是把运行内存中的数据完全同步为当前时间设置指向的外部数据，而命令`Save`则会按当前时间设置把数据保存。

如果存在更改后日期的数据包，更改日期之后会自动读取。
...
```aiignore
成都--昆明+st*GD
# 查找成都到昆明的G和D字头列车，按出发时间排序
```
Sample output #3
```aiignore
成都/成都东/成都西/成都南/犀浦 -> 昆明/昆明南   22 results
1   D251   成都南  	07:06	   昆明   	15:19	< 8:13 >	 成都南 - 昆明 动车
2   G2837  成都东  	07:29	   昆明   	13:25	< 5:56 >	 成都东 - 大理 高速
3   D271   成都南  	08:21	   昆明   	15:59	< 7:38 >	 成都南 - 西双版纳 动车
...
18  G2821  成都东  	16:09	   昆明南  	22:39	< 6:30 >	 成都东 - 昆明南 高速
19  D261   成都南  	16:38	   昆明   	23:40	< 7:02 >	 成都南 - 昆明 动车
20  G2825  成都东  	17:27	   昆明南  	23:44	< 6:17 >	 成都东 - 昆明南 高速
成都/成都东/成都西/成都南/犀浦 -> 昆明/昆明南   22 results, 20 visible
```

你可以输入命令`city_station`，随后对默认的同城车站字典进行编辑，每一次的编辑永久有效。输入`0`表示一个城市的车站输入完毕，输入`save`或`exit`表示退出同城车站的编辑并保存编辑结果。

目前已经上传的同城车站数据涵盖全国地级以上城市。
...
最后，由于代码不太熟练，最初项目的框架考虑的也比较简单，变量都是全局的，所有函数也全部写在了main.py里，望不要嫌弃。
