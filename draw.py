...
def draw_text(object, text="", position_x=0, position_y=0, color=(0, 0, 0), font_size=128, bd=False):
    if bd:
        font = ImageFont.truetype(font_path_bd, font_size)
    else:
        font = ImageFont.truetype(fond_path, font_size)
    text_position = (position_x, position_y)
    object.text(text_position, text, fill=color, font=font)

def rank_station(num=500, key="动分动集普速", prefix="GDCZTKSYP"):
    rank = {}
    station_type = {}
    detail = {}
    for i in train_list:
        ...
        for j in train_list[i]:
            ...
            if (j["station_train_code"][0] == 'G' or
                ...
                train_type = "普速"
            elif j["station_train_code"][0] == 'S':
                train_type = "市域"
            else:
                train_type = "动集"
            station_type[j["station_name"]][train_type] += 1
            if j["station_train_code"][0].isdigit():
                train_prefix = "P"
            else:
    ...
    for r, (station, train_num) in enumerate(rank):
        ...
        draw_text(object=draw, text=str(train_num), position_x=250, position_y=680,
                  font_size=200, bd=True, color=(100, 0, 10))
        draw_text(object=draw, text="动分 " + str(station_type[station]["动分"]), position_x=250, position_y=1050,
                  font_size=90, bd=True, color=(120, 0, 130))
        ...
        position_x = 900
        position_y = 580
        for train_code in detail[station]:
            color = color_blank[train_code[0]]
            draw_text(object=draw, text=train_code, position_x=position_x, position_y=position_y,
                      font_size=font_size, bd=True, color=color)
            line_cnt += 1
            position_x += b_x
            if line_cnt == line:
                position_y += b_y
